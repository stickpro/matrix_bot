...
impl AppError {
    pub fn response(self) -> (StatusCode, AppResponseError) {
        ...
        let (kind, code, details, status_code) = match self {
            ...
            MatrixError(_err) => {
                // Handle ResponseMatrixError specifically
                (
                    "RESPONSE_MATRIX_ERROR".to_string(),
                    None, // Add your specific error code handling here if needed
                    vec![],
                    StatusCode::INTERNAL_SERVER_ERROR,
                )
            },
            ClientBuildError(_err) => (
                "UNKNOWN_ERROR".to_string(),
                None,
                vec![],
                StatusCode::INTERNAL_SERVER_ERROR,
...
#[derive(Debug, strum::EnumString, strum::Display, Copy, Clone, PartialEq, Eq, PartialOrd, Ord)]
